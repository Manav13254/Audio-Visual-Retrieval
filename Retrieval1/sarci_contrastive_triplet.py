...
def get_args():
    ...
    parser.add_argument("--save_path", type=str, default="best_sarci_mim_mixloss_2.pth")
...
class PixelDecoder(nn.Module):
    def __init__(self):
        ...
        self.up1 = nn.Sequential(
            ...
            nn.Conv2d(256, 256, kernel_size=3, padding=1, bias=False), # <-- Added Conv Block
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True)
        ) 
        self.up2 = nn.Sequential(
            nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1),
            nn.Conv2d(128, 128, kernel_size=3, padding=1, bias=False), # <-- Added Conv Block
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True)
        ) 
        self.up3 = nn.Sequential(
            nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1),
            nn.Conv2d(64, 64, kernel_size=3, padding=1, bias=False),   # <-- Added Conv Block
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
        ) 
        self.up4 = nn.Sequential(
            nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=1),
            nn.Conv2d(32, 32, kernel_size=3, padding=1, bias=False),   # <-- Added Conv Block
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True)
        ) 
        self.up5 = nn.Sequential(
            # Upsample while maintaining 32 channels to give the final conv more data to work with
            nn.ConvTranspose2d(32, 32, kernel_size=4, stride=2, padding=1),
            # Final Conv block reduces the 32 channels down to the 3 RGB channels
            nn.Conv2d(32, 3, kernel_size=3, padding=1)                 # <-- Added Final Conv
        ) 
...
class VisionEncoder(nn.Module):
    def __init__(self, embedding_dim=128):
        ...
        # 2. Unfreeze the deep semantic layers
...
# 1. Freeze everything first
        ...
        # 2. Unfreeze the deep semantic blocks
        for param in self.base_model.conv_block5.parameters():
            param.requires_grad = True
...
class CrossModalTripletLoss(nn.Module):
    ...
    def forward(self, v_features, a_features, labels):
        ...
        dist = torch.cdist(v_features, a_features)
        
        labels = labels.view(-1, 1)
        mask_pos = torch.eq(labels, labels.T).float().to(device)
        
        v_a_pos_dist = (dist * mask_pos).max(dim=1)[0]
        v_a_neg_dist = (dist + mask_pos * 1e5).min(dim=1)[0]
        loss_v_a = F.relu(v_a_pos_dist - v_a_neg_dist + self.margin)
        
        dist_t = dist.T
        a_v_pos_dist = (dist_t * mask_pos).max(dim=1)[0]
        a_v_neg_dist = (dist_t + mask_pos * 1e5).min(dim=1)[0]
        loss_a_v = F.relu(a_v_pos_dist - a_v_neg_dist + self.margin)
        
        return (loss_v_a.mean() + loss_a_v.mean()) / 2.0
...
# Validation Routine
def compute_metrics(model, v_dir, a_dir, k_values=[1, 5, 10], batch_size=32):
    model.eval()
...
def main():
    ...
    # Separate parameters into two groups based on their names
    finetune_params = []
    custom_params = []
    ...
    for name, param in model.named_parameters():
        ...
        # If the parameter belongs to the unfreezed backbones
        if 'layer3' in name or 'layer4' in name or 'conv_block5' in name or 'conv_block6' in name:
            finetune_params.append(param)
        else:
            # Everything else (Attention, Decoders, Projection heads)
            custom_params.append(param)

    # Apply a 10x smaller learning rate to the pre-trained weights
    ...
    criterion_triplet = CrossModalTripletLoss(margin=0.7)
    ...
    for epoch in range(max_epochs):
        ...
        for batch_idx, (img_anc, aud_pos, labels) in enumerate(pbar):
            ...
            triplet_loss = criterion_triplet(v_f_pos, a_f_pos, labels)
            recon_loss = criterion_recon(recon_anc, img_anc)
            
            # Reconstruction penalty weight is now constant
            loss = retrieval_loss + triplet_loss + (args.lambda_recon * recon_loss)
            ...
            pbar.set_postfix(
                Loss=f"{loss.item():.3f}", 
                SupCon=f"{retrieval_loss.item():.3f}", 
                Trip=f"{triplet_loss.item():.3f}", 
        ...
        i2a, a2i = compute_metrics(model, VAL_V, VAL_A, batch_size=64   )
