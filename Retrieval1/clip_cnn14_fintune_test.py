...
from datasets import ValVisionDataset, ValAudioDataset
...
# ==========================================
class ClipFinetuneVisionEncoder(nn.Module):
...
class ClipFinetuneAudioEncoder(nn.Module):
    def __init__(self, embedding_dim=128):
        super().__init__()
...
class ClipFinetuneCrossModalModel(nn.Module):
    def __init__(self, embedding_dim=128):
        super().__init__()
        self.vision_model = ClipFinetuneVisionEncoder(embedding_dim)
        self.audio_model = ClipFinetuneAudioEncoder(embedding_dim)
...
def main():
    ...
    model = ClipFinetuneCrossModalModel().to(DEVICE)
    ...
    if os.path.exists(args.model_path):
        ...
        print("Please run train_clip_finetune.py first to generate the model.")
        sys.exit()

    model.eval()
    
    v_ds = ValVisionDataset(VAL_V, transform=clip_transform)
    ...
    print("\n==========================================")
    print("FINAL FINE-TUNED CLIP+CNN14 RESULTS:")
    print("==========================================")
