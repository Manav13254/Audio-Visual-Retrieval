...
def get_args():
    ...
    parser.add_argument("--save_path", type=str, default="best_baseline_model.pth")
...
# ==========================================
class BaselineVisionEncoder(nn.Module):
...
class BaselineCrossModalModel(nn.Module):
    def __init__(self, embedding_dim=128):
        super().__init__()
        self.vision_model = BaselineVisionEncoder(embedding_dim)
        self.audio_model = BaselineAudioEncoder(embedding_dim)


# ==========================================
# BASELINE EVALUATION 
# ==========================================
def compute_baseline_metrics(model, v_dir, a_dir, device, k_values=[1, 5, 10], batch_size=64):
    model.eval()
...
def main():
    ...
    best_avg_r5 = 0.0
    print(f"Starting Baseline Audio-Visual Alignment...")
    ...
    for epoch in range(args.epochs):
        ...
        i2a, a2i = compute_baseline_metrics(model, VAL_V, VAL_A, DEVICE, batch_size=128)
        ...
        if avg_r5 > best_avg_r5:
            ...
            print(f"Best Model Alignment Saved to {args.save_path}")
